...
export function PaginatedReader({ bookId, format, zoom }: PaginatedReaderProps) {
  ...
  // Comics get the real drag-to-curl page turn (and two-page spread) in
  // "flip" mode -- each page is already a flat image, an easy fit for the
  // flip book's leaves. TXT stays on the simple pipeline (reflowable text
  // has no fixed page image). Horizontal/vertical stay one page at a time.
